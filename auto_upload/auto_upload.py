...
station_zipped_folder = 'F:/station_folder/temp'
...
def loop_station():
    ...
    while 1:
        ...
        # print(needed_process_station_zip_namelist)
        # 测试站点 (自发货，销售额小于1500)
        # test_stations_data = pd.read_excel(r"E:\ad_zyp\price_dist\自发货销售额小于1500.xlsx")
        # test_stations = test_stations_data['station'].apply(lambda x: x.upper())
        # needed_process_station_zip_namelist = [zip_station for zip_station in needed_process_station_zip_namelist for
        #                                        station in test_stations if station in zip_station]
        ...
        if len(needed_process_station_zip_namelist) > 0:
            ...
            for one_station_zipped in needed_process_station_zip_namelist:
                ...
                station_dir = os.path.join(station_zipped_folder, os.path.basename(one_station_zipped)[:-4])
                # # main_process_price_range_sku_perf(station_dir)
                try:
                    # 站点新品自动新增
                    print("{} 开始新品自动新增数据...".format(os.path.basename(one_station_zipped)[:-4]))
                    process_new_add_upload_amazon(station_dir)
                    print("{} 完成新品自动新增数据...".format(os.path.basename(one_station_zipped)[:-4]))
                except Exception as e:
                    print(f"{os.path.basename(one_station_zipped)[:-4]}", e)
            shutil.rmtree(station_zipped_folder)
...
def fulfillment_ondates_perf(ad_shop_data, sku_fulfillment_ondates,
                             on_date_range=[-1, 60, 120, 180, 360, 18888888]) -> list:
    """
