...
import change_campaign_sheet_head
...
def init_campaign(initdata, station_abbr, advertised_dir):
    ...
    initdata['ACoS'] = [acos.replace(',','.')  if ',' in str(acos) else acos for acos in initdata['ACoS']]
    initdata['ACoS'] = percent_series_to_numeric(initdata['ACoS'])

    # # 将spend acos sale 中的逗号去掉或变成小数点
    # initdata['Sales'] = initdata.loc[:, 'Sales'].apply(lambda x: x.replace(",",""))
    # initdata['Spend'] = initdata.loc[:, 'Spend'].apply(lambda x: x.replace(",",""))
    # initdata['ACoS'] = initdata.loc[:, 'ACoS'].apply(lambda x: x.replace(",",""))
    # initdata['Orders'] = initdata.loc[:, 'Orders'].apply(lambda x: x.replace(",",""))
    # initdata['Orders'] = initdata.loc[:, 'Orders'].apply(lambda x: x.replace(",",""))
    # initdata['Clicks'] = initdata.loc[:, 'Clicks'].apply(lambda x: x.replace(",",""))
