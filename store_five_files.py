...
def api_push_sku_perf(erpid, erpStationName, sku_perf):
    ...
    headers = {
        ...
        "Host": '121.37.253.64:8001',
    }
    request_url = 'http://121.37.253.64:8001/sales/amazon/SkuPerformance/'
    try:
        requests_data = requests.get(url=request_url, headers=headers, params=params, data=json_data,timeout=(10,60))
...
#         return
#
#     stPerfSaveFolder = r'E:\AD_WEB\file_dir\st_info'
...
def zipped_folders_2_pickle(zipped_files) -> None:
    """
    ...
    unzipped_file_save_folder = r'F:\unzipped_file_temp'
    folder_save_pkl = r"G:\pickle_files"
    delete_file = True
    ...
    redis_store_0 = public_function.Redis_Store(db=0)
        ...
        if 'bulk' in file_path.lower():
            ...
            if camp_sheet_name in read_bulk_file.keys():
                ...
                if not station_cp_data.empty:
                    cpFilePklPath = file_pickle_path_cp
                    ...
                    # 将生成的站点cp表,生成队列到需要处理camp层级信息的redis list中
                    redis_store_0.rpush(commonly_params.station_cp_camp_degree_key,file_redis_key)
            ...
            if '_ac' in file_pickle_path:
                ...
                acFilePklPath = file_pickle_path
            ...
            # 读取站点广告数据
                ...
                stFilePklPath = file_pickle_path
                ...
                brFilePklPath = file_pickle_path
            ...
            # 其他的报表暂时全部直接pickle化,不保存在内存中
                ...
                if ('order' in fileName.lower() or fileName.isdigit()) and ('.csv' in fileType.lower() or 'txt' in fileType.lower()):
                    ...
                    # 筛选出自己站点的数据
                    if 'sales-channel' in station_ao_data.columns:
                        site = station_name[-2:].upper()
                        station_ao_data = station_ao_data[station_ao_data['sales-channel'] == commonly_params.site_web[site]]
    ...
    [redis_store.delete(key) for key in keys if
     (sign_key in key) and (station_name == key[21:-18]) and (last_time not in key)]
    # sub-mission2:站点同时拥有广告报表、st报表、ac报表、br报表
    if ('cpFilePklPath' in vars()) and ('acFilePklPath' in vars()) and ('stFilePklPath' in vars()) and ('brFilePklPath' in vars()):
        station_name = public_function.standardize_station(station_name)
        report_time = datetime.fromtimestamp(last_time_timestamp).strftime('%y.%m.%d')
        redisSaveValue = {station_name:{'cp':cpFilePklPath,'ac':acFilePklPath,'st':stFilePklPath,'br':brFilePklPath,'report_time':report_time}}
        redisSaveValueJson = json.dumps(redisSaveValue)
        redis_store_0.rpush(commonly_params.STATION_SKU_ST_REDIS_KEY,redisSaveValueJson)
        print(f'添加站点{station_name}到st,sku表现处理池中.')
    # 单独的程序跑
    # global report_time
    # report_time = datetime.fromtimestamp(last_time_timestamp).strftime('%y.%m.%d')
...
def refresh_folder(zipped_folder, refresh_before_hour=None):
    """
    判断文件夹中更新压缩文件,
    ...
    while 1:
        ...
        if len(needed_process_station_zip_namelist) > 0:
            ...
            executor = ProcessPoolExecutor(max_workers=PROCESSPOOLNUM)
            all_task = [executor.submit(zipped_folders_2_pickle, (stationZippedFile)) for stationZippedFile in needed_process_station_zip_namelist]
...
if __name__ == "__main__":
    ...
    refresh_folder(zipped_folder,refresh_before_hour=2)
